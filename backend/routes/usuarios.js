...
router.get('/', authMiddleware, requireRole('administrador'), async (req, res) => {
  try {
    ...
    console.error('GET /api/usuarios error:', err && err.stack ? err.stack : err);
...
router.get('/:id', authMiddleware, requireRole('administrador'), async (req, res) => {
  try {
    ...
    console.error('GET /api/usuarios/:id error:', err && err.stack ? err.stack : err);
    res.status(500).json({ error: 'Error interno del servidor' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error('POST /api/usuarios error:', err && err.stack ? err.stack : err);
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error('PUT /api/usuarios/:id error:', err && err.stack ? err.stack : err);
      if (err.code === '23505' || (err.message && err.message.toLowerCase().includes('unique'))) {
        return res.status(409).json({ error: 'El email ya está registrado' });
      }
      res.status(500).json({ error: 'Error interno del servidor' });
...
router.delete('/:id', authMiddleware, requireRole('administrador'), async (req, res) => {
  try {
    ...
    console.error('DELETE /api/usuarios/:id error:', err && err.stack ? err.stack : err);
    res.status(500).json({ error: 'Error interno del servidor' });
  }
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      console.log('LOGIN ATTEMPT email=', String(email).trim().toLowerCase());
      const qres = await pool.query(
      ...
      const rows = await normalizeQueryResult(qres);
      if (!rows || !rows.length) {
        console.log('LOGIN FAIL no user found for', String(email).trim().toLowerCase());
        return res.status(401).json({ error: 'Credenciales inválidas' });
      }
      const usuario = rows[0];
      const hashEnBaseDatos = usuario.contrasena_hash || usuario.contrasenahash || usuario.contraseña_hash;
      if (!hashEnBaseDatos) {
        console.error('LOGIN ERROR missing hash for user id', usuario.id_usuario);
        return res.status(500).json({ error: 'Estructura de usuario inválida' });
      }
      let valid = false;
      try {
        valid = await bcrypt.compare(String(contrasena).trim(), String(hashEnBaseDatos));
      } catch (e) {
        try {
          const bcryptjs = require('bcryptjs');
          valid = await bcryptjs.compare(String(contrasena).trim(), String(hashEnBaseDatos));
        } catch (e2) {
          console.error('BCRYPT compare error:', e && e.stack ? e.stack : e);
          return res.status(500).json({ error: 'Error interno del servidor' });
        }
      }
      if (!valid) {
        console.log('LOGIN FAIL invalid password for user id', usuario.id_usuario);
        return res.status(401).json({ error: 'Credenciales inválidas' });
      }
...
router.post('/auth/refresh', async (req, res) => {
  ...
  try {
    if (!process.env.JWT_REFRESH_SECRET) return res.status(500).json({ error: 'Configuración del servidor incompleta' });
    ...
    console.error('REFRESH EXCEPTION:', err && err.stack ? err.stack : err);
